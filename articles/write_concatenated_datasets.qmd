---
title: "Write concatenated datasets"
format:
  html:
    toc: true
    toc-depth: 3
...
The beginning of the matrix looks like this (Vataireoid legumes, `NTAX=33`, three loci concatenated to `NCHAR=3125`; species missing the first locus are correctly filled with `?`):

![Beginning of a NEXUS-formatted concatenated matrix produced by writeNexus()](../figures/concatenatedmatrix1.svg)

And the end of the file includes the `END;` statement followed by the preliminary MrBayes block, with `charset` definitions for each partition and a `partition regions` command ready to use:

![End of the same NEXUS matrix, showing the auto-generated MrBayes block with charsets for each partition](../figures/concatenatedmatrix2.svg)
...
This is particularly useful when accession labels differ among loci but still need to remain traceable — for example, when a species was sequenced for one locus using a GenBank accession from a different collector than the accession used for another locus.

The output below shows a real multi-accession *Brongniartia* dataset. Notice how GenBank accessions and collector numbers that differ from the "primary" identifier of that row are preserved as bracketed comments, gene by gene, instead of being silently discarded or causing the accession to be treated as an unrelated taxon:

![ITS partition of a concatenated matrix with differing identifiers kept as bracketed comments (e.g. [AF467022], [Hernandez9407_GQ250075])](figures/catGenes_with_taxlabels1.svg)

![ETS partition of the same matrix — the bracketed identifiers change per partition, always reflecting which accession actually contributed data to that gene](../figures/catGenes_with_taxlabels2.svg)
...
![Concatenated PHYLIP-formatted matrix produced by writePhylip()](../figures/concatenatedmatrix3.svg)

And the partition file, ready to upload to RAxML or CIPRES for a mixed/partitioned-model analysis, looks like this:

![RAxML-style partition file produced alongside the PHYLIP matrix](../figures/concatenatedmatrix3_partitition.svg)
