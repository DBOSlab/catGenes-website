...
`catGenes` includes functions to retrieve targeted loci directly from complete organellar genomes available in GenBank:
...
- optionally the original downloaded GenBank files, if `rm.gb.files = FALSE`
...
By default, downloaded `.gb` files can be kept or removed depending on the setting of `rm.gb.files`.
...
- if the same locus was also mined directly with `mineSeq()`/`mineTaxa()`, combine both same-locus files into one with `combineFASTA()` before alignment — see [Combining the same locus mined from different sources](/articles/combine_and_align_sequences.qmd#combining-the-same-locus-mined-from-different-sources)
